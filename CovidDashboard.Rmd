...
```{r}
yesterday_new_cases %>%
  valueBox(caption='Yesterday New Cases',icon="nauseated_face")
```
...
```{r Washington Daily Vacinations}
...
format(wa_cases,big.mark = ",") %>%
  valueBox(caption='Cumulative Cases',icon=emoji("nauseated_face"))
```
...
### Cumulative US Cases
```{r Cumulative US Cases,  echo=FALSE, message = FALSE, out.width="100%"}
nytimes %>%
  group_by(date) %>%
...
si_number(us_cases,3) %>%
  valueBox(caption='Cumulative Cases',icon="fa-disease")
```
...
The New York Times. (2021). Coronavirus (Covid-19) Data in the United States. Retrieved `r Sys.time()``, from  [https://github.com/nytimes/covid-19-data](https://github.com/nytimes/covid-19-data)
...
United States Census Bureau, State and Conty FIPS identifiers. [https://www2.census.gov/geo/docs/reference/codes/files/national_county.txt](https://www2.census.gov/geo/docs/reference/codes/files/national_county.txt)

United States Census Bureau, County Population Projections. [https://www2.census.gov/programs-surveys/popest/datasets/2010-2019/counties/totals/co-est2019-alldata.csv](https://www2.census.gov/programs-surveys/popest/datasets/2010-2019/counties/totals/co-est2019-alldata.csv)
